...
module.exports = function vds_query(task,next){
    ...
    if(task.id !== undefined || task.years !== undefined){
        query.push('where')
    }
    if(task.id){
        query.push('id='+task.id )
    }
    if(task.id !== undefined && task.years !== undefined){
        query.push('and')
    }
    if(task.years){
        if(Array.isArray(task.years)){
            query.push('extract( year from vv.version) in ('+task.years.join(',')+')')
        }else{
            query.push('extract( year from vv.version) >= '+task.years)
        }
    }
